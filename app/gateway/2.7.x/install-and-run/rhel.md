...
{% navtab Kong Gateway %}
```bash
curl -Lo kong-enterprise-edition-{{page.versions.ee}}.rpm $(rpm --eval {{ site.links.cloudsmith }}/public/gateway-legacy/rpm/el/%{rhel}/x86_64/kong-enterprise-edition-{{page.versions.ee}}.rhel%{rhel}.noarch.rpm)
```
{% endnavtab %}
{% navtab Kong Gateway (OSS) %}
```bash
curl -Lo kong-{{page.versions.ce}}.rpm $(rpm --eval {{ site.links.cloudsmith }}/public/gateway-legacy/rpm/el/%{rhel}/x86_64/kong-{{page.versions.ce}}.rhel%{rhel}.amd64.rpm)
 ```
{% endnavtab %}
{% endnavtabs_ee %}
{% endcapture %}
